...
(function () {
  ...
  var CAT = {  // categorical, fixed slot order (blue, aqua, yellow, green, violet, red, magenta, orange)
    light: ["#2a78d6", "#1baf7a", "#eda100", "#008300", "#4a3aa7", "#e34948", "#e87ba4", "#eb6834"],
    dark:  ["#3987e5", "#199e70", "#c98500", "#008300", "#9085e9", "#e66767", "#d55181", "#d95926"]
  };
  var ORD = {  // ordinal blue ramp for n-sweeps (light->dark = more data)
  ...
  function withECharts(cb) {
    ...
    s.onload = function () { loading.forEach(function (f) { f(); }); loading = null; };
    document.head.appendChild(s);
  }
  ...
  function render(id, build) {
    ...
    withECharts(function () {
      ...
      function paint() { chart.setOption(build(ctx()), true); }
      paint();
  ...
  function base(c, title, subtext) {
    return {
      ...
      title: {
        text: title, subtext: subtext, left: "center",
  ...
  FIG["expdisc-fig-dilution"] = function () {
    var DATA = {
      ...
      topsources: {
        "1": [["LogD", 29], ["plasma bind.", 17], ["Caco-2 Papp", 11], ["Caco-2 efflux", 11]],
        "2": [["LogD", 48], ["plasma bind.", 25], ["muscle bind.", 24], ["Caco-2 Papp", 21]],
        "3": [["LogD", 63], ["muscle bind.", 37], ["brain bind.", 36], ["plasma bind.", 31]],
        "4": [["LogD", 69], ["plasma bind.", 48], ["brain bind.", 46], ["muscle bind.", 45]],
        "5": [["LogD", 75], ["muscle bind.", 63], ["brain bind.", 63], ["solubility", 54]],
        "6": [["LogD", 81], ["muscle bind.", 71], ["Caco-2 Papp", 69], ["brain bind.", 69]],
        "7": [["LogD", 87], ["brain bind.", 80], ["plasma bind.", 80], ["muscle bind.", 79]],
        "8": [["Caco-2 Papp", 89], ["HLM cl.", 89], ["solubility", 89], ["LogD", 89]]
      }
    };
    var NS = ["5", "10", "25", "50"];
    return function (c) {
      var o = base(c,
        "The best subset is small, then extra sources dilute",
        "best-of-size-k, averaged over 9 targets");
      o.legend = { top: 46, textStyle: { color: c.ink }, data: NS.map(function (n) { return "n = " + n; }) };
      ...
      o.tooltip.formatter = function (ps) {
        ...
        if (t.length) {
          ...
          s += '<span style="opacity:.9">' + t.map(function (x) { return x[0] + " " + x[1] + "%"; }).join(" · ") + "</span>";
        }
        return s;
      };
      o.xAxis = {
        type: "category", data: DATA.sizes, boundaryGap: false,
        name: "sources in the subset (k)", nameLocation: "middle", nameGap: 30,
        axisLine: { lineStyle: { color: c.grid } }, axisTick: { show: false },
        axisLabel: { color: c.ink }, nameTextStyle: { color: c.ink }
      };
      o.yAxis = {
        type: "value", name: "rank correlation ρ (test)", min: 0.4, max: 0.57,
      ...
      o.series = NS.map(function (n, i) {
        return {
          name: "n = " + n, type: "line", smooth: true, symbolSize: 7,
          lineStyle: { width: 2.6, color: c.ord[i] }, itemStyle: { color: c.ord[i] },
          emphasis: { focus: "series" }, data: DATA.series[n],
          markArea: i === 0 ? {
            silent: true, itemStyle: { color: c.mode === "dark" ? "rgba(144,133,233,0.14)" : "rgba(74,58,167,0.07)" },
  ...
  var LADDER = {  // OpenADMET mean test Spearman over 9 targets (paper Table 1)
    floor:   [0.053, 0.076, 0.091, 0.115],
    random:  [0.202, 0.329, 0.350, 0.353],
    all:     [0.390, 0.468, 0.482, 0.480],
    corr:    [0.457, 0.509, 0.531, 0.508],
    agent:   [0.495, 0.549, 0.530, 0.508],
  ...
  function ladderLine(name, data, color, opts) {
    ...
    return {
      ...
      itemStyle: { color: color }, z: opts.z || 2, emphasis: { focus: "series" }
    };
  }
  function ladder(c, title, subtext, withAgent) {
    ...
    o.tooltip.formatter = function (ps) {
      ...
      ps.slice().sort(function (a, b) { return (b.value || 0) - (a.value || 0); })
        .forEach(function (p) {
    ...
    o.xAxis = {
      type: "category", data: LADDER_N, boundaryGap: false,
      name: "few-shot budget (labels on the new assay)", nameLocation: "middle", nameGap: 30,
      axisLine: { lineStyle: { color: c.grid } }, axisTick: { show: false },
      axisLabel: { color: c.ink, formatter: function (v) { return "n=" + v; } },
      nameTextStyle: { color: c.ink }
    };
    o.yAxis = {
      type: "value", name: "rank correlation ρ (test)", min: 0, max: 0.66,
    ...
    var ceil = {
      name: "ceiling (full labels)", type: "line", data: [null, null, null, null],
      lineStyle: { width: 0 }, itemStyle: { color: c.inkSoft },
      markLine: {
  ...
  FIG["expdisc-fig-baselines"] = function () {
    return function (c) {
      return ladder(c,
        ...
        "OpenADMET, mean test ρ over 9 targets (no agent yet)", false);
    };
  };
  FIG["expdisc-fig-methods"] = function () {
    return function (c) {
      return ladder(c,
        "The agent reaches the oracle and beats naive pooling",
        "OpenADMET, mean test ρ over 9 targets", true);
    };
  };
  ...
  function linesOption(c, cfg) {
    ...
    o.tooltip.formatter = function (ps) {
      ...
      ps.slice().sort(function (a, b) { return (b.value || 0) - (a.value || 0); })
        .forEach(function (p) { if (p.value != null) s += p.marker + p.seriesName + ": <b>" + (+p.value).toFixed(3) + "</b><br/>"; });
      return s;
    };
    o.xAxis = {
      type: "category", data: cfg.xlabels, boundaryGap: false,
      name: cfg.xaxisName, nameLocation: "middle", nameGap: 30,
      axisLine: { lineStyle: { color: c.grid } }, axisTick: { show: false },
      axisLabel: { color: c.ink, formatter: cfg.xfmt }, nameTextStyle: { color: c.ink }
    };
    o.yAxis = {
      type: "value", name: cfg.yname, min: cfg.ymin, max: cfg.ymax,
      nameLocation: "middle", nameRotate: 90, nameGap: 46,
      axisLine: { show: false }, splitLine: { lineStyle: { color: c.grid } },
      axisLabel: { color: c.ink }, nameTextStyle: { color: c.ink }
    };
  ...
  function errorBars(name, means, sems, color, xoffPx) {
    return {
      name: name, type: "custom", silent: true, z: 6, tooltip: { show: false },
      data: means.map(function (m, i) { return [i, m - sems[i], m + sems[i]]; }),
      renderItem: function (params, api) {
        var lo = api.coord([api.value(0), api.value(1)]);
        var hi = api.coord([api.value(0), api.value(2)]);
        var x = lo[0] + (xoffPx || 0), cap = 4;
  ...
  FIG["expdisc-fig-negtransfer"] = function () {
    var D = { floor: -0.013, bars: [["plasma bind.", 0.566], ["HLM cl.", 0.436], ["muscle bind.", 0.33], ["solubility", 0.18], ["brain bind.", 0.165], ["MLM cl.", -0.001], ["Caco-2 Papp", -0.017], ["Caco-2 efflux", -0.09]] };
    return function (c) {
      var o = base(c, "One source at a time: the right assay lifts LogD, the wrong one hurts",
      ...
      o.tooltip.formatter = function (p) {
        return "<b>" + p.name + "</b><br/>ρ = <b>" + (+p.value).toFixed(3) + "</b><br/>" +
          '<span style="opacity:.7">' + (p.value >= D.floor ? "helps — above no transfer" : "hurts — below no transfer") + "</span>";
      };
      o.xAxis = {
        type: "category", data: D.bars.map(function (b) { return b[0]; }),
        axisLine: { lineStyle: { color: c.grid } }, axisTick: { show: false },
        axisLabel: { color: c.ink, interval: 0, rotate: 30 }
      };
      o.yAxis = {
        type: "value", name: "rank correlation ρ (test)", nameLocation: "middle", nameRotate: 90, nameGap: 44,
        axisLine: { show: false }, splitLine: { lineStyle: { color: c.grid } },
        axisLabel: { color: c.ink }, nameTextStyle: { color: c.ink }
      };
      o.series = [{
  ...
  FIG["expdisc-fig-coverage"] = function () {
    var D = [["KSOL (solubility)", 96, 5128, "source"], ["LogD", 95, 5039, "source"], ["MLM clearance", 85, 4522, "source"],
      ["HLM clearance", 71, 3759, "source"], ["Caco-2 Papp", 40, 2157, "source"], ["Caco-2 efflux", 41, 2161, "source"],
      ["plasma binding", 24, 1302, "target"], ["brain binding", 18, 975, "target"], ["muscle binding", 4, 222, "target"]];
    return function (c) {
      var o = base(c, "One dataset, a steep coverage gradient",
        "OpenADMET: % of training molecules with a measured label");
      o.grid = { left: 118, right: 60, top: 88, bottom: 52 };
      o.tooltip.trigger = "item";
      o.tooltip.formatter = function (p) {
        var row = D[p.dataIndex];
        return "<b>" + row[0] + "</b><br/>" + row[1] + "% measured (" + row[2].toLocaleString() + " molecules)<br/>" +
          '<span style="opacity:.7">' + (row[3] === "source" ? "data-rich → transfer source" : "data-poor → few-shot target") + "</span>";
      };
      o.legend = {
        top: 46, textStyle: { color: c.ink },
        data: [{ name: "data-rich source" }, { name: "data-poor target" }]
      };
      o.xAxis = {
        type: "value", max: 100, name: "% of molecules measured", nameLocation: "middle", nameGap: 26,
        axisLine: { show: false }, splitLine: { lineStyle: { color: c.grid } },
        axisLabel: { color: c.ink, formatter: "{value}%" }, nameTextStyle: { color: c.ink }
      };
      o.yAxis = {
        type: "category", inverse: true, data: D.map(function (r) { return r[0]; }),
        axisLine: { lineStyle: { color: c.grid } }, axisTick: { show: false }, axisLabel: { color: c.ink }
      };
      function bars(role, color, name) {
        return {
          name: name, type: "bar", barWidth: "62%", stack: "x", color: color,
          data: D.map(function (r) { return r[3] === role ? { value: r[1], itemStyle: { borderRadius: 3 } } : "-"; }),
          label: { show: true, position: "right", color: c.inkSoft, formatter: function (p) { return D[p.dataIndex][1] + "%"; } }
        };
      }
  ...
  FIG["expdisc-fig-tdc-baselines"] = function () {
    var D = { ns: [10, 25, 50], floor: [0.141, 0.206, 0.254], random: [0.124, 0.124, 0.137],
      all: [0.202, 0.185, 0.189], heuristic: [0.204, 0.24, 0.236], agent_full: [0.225, 0.253, 0.296],
      sem: { floor: [0.032, 0.026, 0.033], random: [0.026, 0.017, 0.019], all: [0.030, 0.035, 0.034],
             heuristic: [0.046, 0.035, 0.046], agent_full: [0.045, 0.040, 0.049] } };
    return function (c) {
      var rows = [
      ...
      var lines = rows.map(function (r) { return ladderLine(r[1], r[2], r[3], r[4]); });
      var bars = rows.map(function (r, i) { return errorBars(r[1], r[2], D.sem[r[0]], r[3], (i - (rows.length - 1) / 2) * 5); });
      return linesOption(c, {
        ...
        xlabels: D.ns, xname: "n", xaxisName: "few-shot budget (labels on the new task)",
        xfmt: function (v) { return "n=" + v; }, yname: "rank correlation ρ (test)",
        ymin: 0.08, ymax: 0.35, legendRight: 24,
  ...
  FIG["expdisc-fig-tdc"] = function () {
    var D = { ns: [10, 25, 50], heuristic: [0.204, 0.24, 0.236],
      agent_corr: [0.241, 0.281, 0.217], agent_meta: [0.204, 0.239, 0.282], agent_full: [0.225, 0.253, 0.296],
      sem: { heuristic: [0.046, 0.035, 0.046], agent_corr: [0.040, 0.030, 0.050],
             agent_meta: [0.055, 0.036, 0.049], agent_full: [0.045, 0.040, 0.049] } };
    return function (c) {
      var rows = [
      ...
      return linesOption(c, {
        ...
        xlabels: D.ns, xname: "n", xaxisName: "few-shot budget (labels on the new task)",
        xfmt: function (v) { return "n=" + v; }, yname: "rank correlation ρ (test)",
        ymin: 0.1, ymax: 0.35, legendRight: 24,
        series: lines.concat(bars)
      });
    };
  };
  ...
  FIG["expdisc-fig-autoresearch"] = function () {
    var D = { targets: [["MGMB", 222], ["MBPB", 975], ["MPPB", 1302]],
      floor: [0.518, 0.493, 0.321], greedy: [0.722, 0.719, 0.657], agent: [0.714, 0.686, 0.675], oracle: [0.747, 0.72, 0.706] };
    return function (c) {
      var o = base(c, "One autonomous agent reaches 96% of the brute-force oracle",
      ...
      o.tooltip.formatter = function (ps) {
        return "<b>" + ps[0].axisValue.replace("\n", " ") + "</b><br/>" +
          ps.map(function (p) { return p.marker + p.seriesName + ": <b>" + (+p.value).toFixed(3) + "</b>"; }).join("<br/>");
      };
      o.xAxis = {
        type: "category", data: D.targets.map(function (t) { return t[0] + "\n(" + t[1] + " labels)"; }),
        axisLine: { lineStyle: { color: c.grid } }, axisTick: { show: false }, axisLabel: { color: c.ink }
      };
      o.yAxis = {
        type: "value", name: "rank correlation ρ (test)", max: 0.82, nameLocation: "middle", nameRotate: 90, nameGap: 44,
        axisLine: { show: false }, splitLine: { lineStyle: { color: c.grid } },
        axisLabel: { color: c.ink }, nameTextStyle: { color: c.ink }
      };
      function bar(name, data, color, opts) {
        return { name: name, type: "bar", data: data, color: color,
          itemStyle: { borderRadius: 3, borderColor: c.surface, borderWidth: 1 }, z: (opts && opts.z) || 2 };
      }
      o.series = [
  ...
  FIG["expdisc-fig-coldstart"] = function () {
    var D = { ns: [5, 10, 25, 50],
      ...
      sem: {
        ...
        agent_permol:  [0.044, 0.043, 0.041, 0.040],
    ...
    return function (c) {
      ...
      return linesOption(c, {
        ...
        xlabels: D.ns, xname: "molecules measured", xaxisName: "molecules measured first (cold start)",
        xfmt: function (v) { return "n=" + v; }, yname: "rank correlation ρ (test)",
        ymin: 0.24, ymax: 0.58, legendRight: 24,
        series: lines.concat(bars)
      });
    };
  };
